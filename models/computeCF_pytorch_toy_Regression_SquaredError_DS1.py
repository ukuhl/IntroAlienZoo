...
if __name__ == "__main__":
    ...
    # load data
    df = pd.read_csv("AlienZooDataSet1.csv")
    ...
    # save trained model
    print("save model")
    torch.save(net,'PyTorchNetDataSet1_Regression_500epochs_SquaredError.joblib')
    ## ### ### ### ###

    # load the model
    print("load model")
    net = torch.load('PyTorchNetDataSet1_Regression_500epochs_SquaredError.joblib')
    ...
    for t in range(0, 20):
        try:
            ...
            ## NEW:
            #ttest=1.9
            #x_orig=x_test[t].numpy()
            #print("prediction on current item:")
            #print(net(x_orig))
            ###############
            ...
            cf, y_cf, delta = generate_counterfactual(net, x_orig, y_target=ttest, features_whitelist=features_whitelist, C=[0.001, 0.01, 0.1, 1.0], regularization="l2", optimizer="nelder-mead", done=done, return_as_dict=False)
            print("cf")
            print(cf)
            print(x_orig)
            print(np.round(cf))
