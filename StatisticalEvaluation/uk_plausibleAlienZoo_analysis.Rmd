...
Finally, we *do not formulate a prediction whether users will uncover inconsistencies in the feedback* (maybe that happens in case of "closest" CFEs when we're in the areas of "no training data“?) (questionnaire item 7). This will be investiagted in a further exploratory analysis.
...
# Where's the data
perf_source = list.files(pattern="PerformanceSim.csv")
rt_source = list.files(pattern="ReactionTimesSim.csv")
survey_source = list.files(pattern="SurveySim.csv")
attention_source = list.files(pattern="AttentionCheckSim.csv")
...
min_timeAgreementScene=10000 # 10 seconds?
min_timeStartScene=5000 # 5 seconds?
min_timeStableUntilFeeding=5000 # 5 seconds?
min_timeFeedbackScene=10000 # 10 seconds?
...
pRTagreement <- ggplot(unique(df_rt[,c('userId','timeAgreementScene','group')]), aes(x=timeAgreementScene, y=0,label=userId,colour = factor(group)))+ 
  ...
  geom_label_repel(data = . %>% mutate(lab = ifelse(timeAgreementScene < 
    min_timeAgreementScene, levels(df_rt$userId)[userId], "")),
                  aes(label = lab), 
                  box.padding = 1,
...
pRTstart <- ggplot(unique(df_rt[,c('userId','timeStartScene','group')]), aes(x=timeStartScene, y=0,label=userId,colour = factor(group)))+ 
  ...
  geom_label_repel(data = . %>% mutate(lab = ifelse(timeStartScene < 
    min_timeStartScene, levels(df_rt$userId)[userId], "")),
                  aes(label = lab), 
                  box.padding = 1,
                  show.legend = FALSE,max.overlaps = Inf)+
...
pRTuntilFeeding <- ggplot(df_rt[,c('userId','timeStableUntilFeeding','group','TrialNr')], aes(x=TrialNr, y=timeStableUntilFeeding,group=userId,label=userId,colour = factor(group)))+ 
  ...
  geom_label_repel(data = . %>% mutate(lab = ifelse(timeStableUntilFeeding < min_timeStableUntilFeeding,levels(df_rt$userId)[userId], "")),
                  aes(label = lab), 
                  box.padding = 1,
...
pRTfeedback <- ggplot(unique(df_rt[,c('userId','timeFeedbackScene','group','BlockNr')]), aes(x=BlockNr, y=timeFeedbackScene,group=userId,label=userId,colour = factor(group)))+ 
  ...
  geom_label_repel(data = . %>% mutate(lab = ifelse(timeFeedbackScene < min_timeFeedbackScene,levels(df_rt$userId)[userId], "")),
                  aes(label = lab), 
                  box.padding = 1,
                  show.legend = FALSE,max.overlaps = Inf) + #this removes the 'a' from the legend
...
# save
ggsave("Figures/DatQual_RTquality_Agree_Start.pdf",width = 9, height = 7,)

# put plots together, v2
figure_RTquality_Feeding_Feedback <- ggarrange(pRTuntilFeeding, pRTfeedback,
                    ncol = 1, nrow = 2, align = "v")
# save
ggsave("Figures/DatQual_RTquality_Feeding_Feedback.pdf",width = 9, height = 7,)
...
speederFeeding_IDs=as.character(unique(df_rt$userId[df_rt$timeStableUntilFeeding < min_timeStableUntilFeeding]))
...
straightlineGame_data=merge(merge(merge(merge(
...
straightlineGame_data=straightlineGame_data[(!straightlineGame_data$mismatchP1) & (!straightlineGame_data$mismatchP2) & (!straightlineGame_data$mismatchP3) & (!straightlineGame_data$mismatchP4) & (!straightlineGame_data$mismatchP5), ]
...
# get checked values for items 3-9
straightlineSurvey_data=dt_survey[ checked == 1 & itemNo > 2] 

# identify users that answered only 1 or 2 distinct responses throughout all questions
straightlinersSurvey_IDs=straightlineSurvey_data[,list(mismatchSurveyItems=length(unique(responseNo))>2),keyby=.(userId)]
# keep only users without "mismatchSurveyItems" (no mismatch = uniform answers)
straightlinersSurvey_IDs=straightlinersSurvey_IDs[!straightlinersSurvey_IDs$mismatchSurveyItems]
...
# remove attentionFailers, that were not already recognized as speeders
...
straightlinersGame_IDs_clean=straightlinersGame_IDs[!straightlinersGame_IDs %in% c(speeder_IDs,attentionFailBoth_IDs_clean) ]
...
# remove survey straightliners, that were not already recognized as speeders / attention failers / game straightliners

straightlinersSurvey_IDs_clean=straightlinersSurvey_IDs[!straightlinersSurvey_IDs %in% c(speeder_IDs,attentionFailBoth_IDs_clean,straightlinersGame_IDs_clean) ]
...
* we remove `r length(attentionFailBoth_IDs_clean)` users that failed both attention tests
* we remove `r length(straightlinersGame_IDs_clean)` users that straightlined in the game
...
Do we have an equal number of users in each clean dataframe? `r length(unique(userNo_raw$Matches))==1`
...
H1.1_p_ShubsPerTrial <- ggplot(df_perf, aes(x=trialNo, y=shubNoNew, group = userId, color= group))+ 
  ...
  scale_x_continuous(breaks=1:max(df_perf$trialNo))+
  scale_colour_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
...
ShubNo_effect= lmer(shubNoNew ~ group*trialNo + (1|userId), data = df_perf) # linear model DV ShubNoNew predicted by the IV (trials, i.e. time)
...
* main effect of group: F(`r ShubNo_effect_anova$NumDF[1]`,`r ShubNo_effect_anova$DenDF[1]`)=`r ShubNo_effect_anova$Fvalue[1] `, p=`r ShubNo_effect_anova$Pvalue[1]`,$\eta_{\text{p}}^{2}$=`r ShubNo_effect_effsize$Eta2_partial[1]`

* main effect of trials (time): F(`r ShubNo_effect_anova$NumDF[2]`,`r ShubNo_effect_anova$DenDF[2]`)=`r ShubNo_effect_anova$Fvalue[2] `, p=`r ShubNo_effect_anova$Pvalue[2]`,$\eta_{\text{p}}^{2}$=`r ShubNo_effect_effsize$Eta2_partial[2]`

* interaction (group x trials): F(`r ShubNo_effect_anova$NumDF[3]`,`r ShubNo_effect_anova$DenDF[3]`)=`r ShubNo_effect_anova$Fvalue[3] `, p=`r ShubNo_effect_anova$Pvalue[3]`,$\eta_{\text{p}}^{2}$=`r ShubNo_effect_effsize$Eta2_partial[3]`
...
H1.2_p_RTPerTrial <- ggplot(df_rt, aes(x=TrialNr, y=timeStableUntilFeeding, group = userId, color= group))+ 
  ...
  geom_line()+
  #facet_wrap(vars(group),nrow = 2, ncol = 1)+
  labs(title="Development of reaction times by group over trials",x="Trial", y = "Reaction time (ms)")+
  theme_bw(base_size = 10)+
  #scale_y_continuous(limits = c(0, 100))+
  scale_x_continuous(breaks=1:max(df_rt$TrialNr))+
  scale_colour_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  theme(legend.position="bottom")
...
H1.2_p_RTPerBlock <- ggplot(df_rt_blockStats, aes(x=BlockNr, y=timeStableUntilFeeding[,"mean"], group = userId, color= group))+ 
  ...
  #facet_wrap(vars(group),nrow = 2, ncol = 1)+
  labs(title="Development of reaction times by group over blocks",x="Block", y = "Reaction time (ms)")+
  theme_bw(base_size = 10)+
  #scale_y_continuous(limits = c(0, 100))+
  scale_x_continuous(breaks=1:max(df_perf$blockNo))+
  scale_colour_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
...
# save
ggsave("Figures/H1.2_figure1_RTData.pdf",width = 5, height = 4,)
...
# save
ggsave("Figures/H1.3_figure2_UserResponseMatches.pdf",width = 5, height = 6,)
...
H2.1_p_FreqUserResponses = ggplot(data=df_survey_subjUsability_summary, aes(x=responseNo,fill = group)) + 
  ...
  labs(title="Judged helpfulness / usability of CFEs",x="", y = "Frequency of answer")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
...
H2.1_p_PercUserResponses = ggplot(data=df_survey_subjUsability_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = PercUsersChecked),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`5`="\"Feedback helped me.\"",`6`="\"I could use the feedback.\"")))+
  labs(title="Judged helpfulness / usability of CFEs",x="", y = "% of users per group")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_x_discrete(breaks=1:6, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree","Prefer not\nto answer"))+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 60,hjust = 0.95))
...
# save
ggsave("Figures/H2.1_figure1_UserResponses.pdf",width = 5, height = 6,)
...
# plot distribution
H2.1_figure2_UserResponses = ggplot(df_survey_subjHelpUsability_responses, aes(x=group, y=responseNo, color=group, fill=group)) + 
...
# save
ggsave("Figures/H2.1_figure2_UserResponses.pdf",width = 5, height = 6,)

# show plot
print("Mean user response for subjective helpfulness / usability:")
H2.1_figure2_UserResponses
...
# Descriptive stats
# subset to get only relevant data
df_survey_UsabilityForOthers=df_survey[df_survey$itemNo==8,]
...
H2.2_p_FreqUserResponses = ggplot(data=df_survey_UsabilityForOthers_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = SumChecks),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`8`="\"Most people would learn quickly.\"")))+
  labs(title="Judged helpfulness for others",x="", y = "Frequency of answer")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_x_discrete(breaks=1:6, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree","Prefer not\nto answer"))+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 60,hjust = 0.95))
...
H2.2_p_PercUserResponses = ggplot(data=df_survey_UsabilityForOthers_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = PercUsersChecked),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`8`="\"Most people would learn quickly.\"")))+
  labs(title="Judged helpfulness for others",x="", y = "% of users per group")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_x_discrete(breaks=1:6, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree","Prefer not\nto answer"))+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 60,hjust = 0.95))
...
# save
ggsave("Figures/H2.2_figure1_UserResponses.pdf",width = 5, height = 6,)
...
# plot distribution
H2.2_figure2_UserResponses = ggplot(df_survey_UsabilityForOthers_responses, aes(x=group, y=responseNo, color=group, fill=group)) +
  geom_violin(scale = "count")+
  geom_boxplot(width=0.1)+
  #geom_jitter(height = 0, width = 0.1)+ # ADD THIS TO DISPLAY INDIVIDUAL DATA POINTS!
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`8`="\"Most people would learn quickly.\""))) +
  stat_summary(fun=mean, geom="point", shape=23, size=2, color="red")  +
  labs(title="Judged helpfulness for others",x="Group", y = "Response")+
  scale_y_continuous(limits = c(1, 5),breaks=1:5, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree"))+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=alpha(c(Ccol,Pcol),0.2), labels = c("Closest", "Plausible"))+
  scale_color_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
...
H3_FreqUserResponses = ggplot(data=df_survey_understanding_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = SumChecks),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`3`="\"I understood the feedback.\"",`4`="\"I did not need support to understand.\"")))+
  labs(title="Judged understanding of CFEs",x="", y = "Frequency of answer")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_x_discrete(breaks=1:6, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree","Prefer not\nto answer"))+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 60,hjust = 0.95))
...
H3_PercUserResponses = ggplot(data=df_survey_understanding_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = PercUsersChecked),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`3`="\"I understood the feedback.\"",`4`="\"I did not need support to understand.\"")))+
  labs(title="Judged understanding of CFEs",x="", y = "% of users per group")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_x_discrete(breaks=1:6, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree","Prefer not\nto answer"))+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 60,hjust = 0.95))
...
# save
ggsave("Figures/H3_figure1_UserResponses.pdf",width = 5, height = 6,)
...
# plot distribution
H3_figure2_UserResponses = ggplot(df_survey_understanding_support_responses, aes(x=group, y=responseNo, color=group, fill=group)) + 
  geom_violin(scale = "count")+
  geom_boxplot(width=0.1)+
  #geom_jitter(height = 0, width = 0.1)+ # ADD THIS TO DISPLAY INDIVIDUAL DATA POINTS!
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`3`="\"I understood the feedback.\"",`4`="\"I did not need support to understand.\""))) +
...
# save
ggsave("Figures/H3_figure2_UserResponses.pdf",width = 5, height = 6,)

# show plot
print("Mean user response for understanding / need for support to understand:")
H3_figure2_UserResponses
...
# Descriptive stats
# subset to get only relevant data
df_survey_TimingEfficacy=df_survey[df_survey$itemNo==9,]
...
H4_FreqUserResponses = ggplot(data=df_survey_TimingEfficacy_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = SumChecks),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`9`="\"I received feedback timely and efficiently.\"")))+
  labs(title="Judged presentation of CFEs",x="", y = "Frequency of answer")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_x_discrete(breaks=1:6, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree","Prefer not\nto answer"))+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 60,hjust = 0.95))
...
H4_PercUserResponses = ggplot(data=df_survey_TimingEfficacy_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = PercUsersChecked),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`9`="\"Feedback was timely and efficiently.\"")))+
  labs(title="Judged presentation of CFEs",x="", y = "% of users per group")+
  theme_bw(base_size = 10)+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_x_discrete(breaks=1:6, labels=c("Strongly\ndisagree","Disagree","Neutral","Agree","Strongly\nagree","Prefer not\nto answer"))+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 60,hjust = 0.95))
...
# save
ggsave("Figures/H4_figure1_UserResponses.pdf",width = 5, height = 6,)
...
# plot distribution
H4_figure2_UserResponses = ggplot(df_survey_TimingEfficacy_responses, aes(x=group, y=responseNo, color=group, fill=group)) +
  geom_violin(scale = "count")+
  geom_boxplot(width=0.1)+
  #geom_jitter(height = 0, width = 0.1)+ # ADD THIS TO DISPLAY INDIVIDUAL DATA POINTS!
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`9`="\"Feedback was timely and efficiently.\""))) +
...
# save
ggsave("Figures/H4_figure2_UserResponses.pdf",width = 5, height = 6,)

# show plot
print("Mean user response for subjective helpfulness / usability:")
H4_figure2_UserResponses

# run stats, helpfulness judgements:
item9t="Wilcox"
item9test=uk_wilcox.test(df_survey_TimingEfficacy_responses$responseNo[df_survey_TimingEfficacy_responses$group=="P"],df_survey_TimingEfficacy_responses$responseNo[df_survey_TimingEfficacy_responses$group=="C"],paired=FALSE,exact=FALSE)
item9effsize=item9test$z_val/(sqrt(nrow(df_survey_TimingEfficacy_responses)))

matchingRes=paste(matchingRes,paste("\n","H4_UserJudgementsHelpfulness",sep=""),"NA",item9t,item9test$p.value,item9effsize,sep = ",")

```

The analysis revealed:

* Is there a significant difference in terms of estimated usefulness for others between groups? We compared number of matches for users in plausible condition (M = `r df_survey_TimingEfficacy_summary$mean[df_survey_TimingEfficacy_summary$group=="P"]`, SD = `r df_survey_TimingEfficacy_summary$sd[df_survey_TimingEfficacy_summary$group=="P"]`) and users in the closest condition (M = `r df_survey_TimingEfficacy_summary$mean[df_survey_TimingEfficacy_summary$group=="C"]`, SD = `r df_survey_TimingEfficacy_summary$sd[df_survey_TimingEfficacy_summary$group=="C"]`) using a Wilcoxon–Mann–Whitney U test. This showed: U=`r item9test$statistic `, p=`r item9test$p.value `, r = `r item9effsize `
...
# Descriptive stats
# subset to get only relevant data
df_survey_Inconsist=df_survey[df_survey$itemNo==7,]
...
Expl_FreqUserResponses = ggplot(data=df_survey_Inconsist_summary, aes(x=responseNo,fill = group)) + 
  geom_bar(aes(y = SumChecks),stat="identity",position = position_dodge(preserve = "single"))+
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`7`="\"I did not find inconsistencies.\"")))+
...
# save
ggsave("Figures/Expl_figure1_UserResponses.pdf",width = 5, height = 6,)
...
# plot distribution
Expl_figure2_UserResponses = ggplot(df_survey_Inconsist_responses, aes(x=group, y=responseNo, color=group, fill=group)) +
  geom_violin(scale = "count")+
  geom_boxplot(width=0.1)+
  #geom_jitter(height = 0, width = 0.1)+ # ADD THIS TO DISPLAY INDIVIDUAL DATA POINTS!
  facet_wrap(vars(itemNo),nrow = 2, ncol = 1,labeller=as_labeller(c(`7`="\"I did not find inconsistencies.\""))) +
...
# save
ggsave("Figures/Expl_figure2_UserResponses.pdf",width = 5, height = 6,)

# show plot
print("Mean user response for inconsistencies of CFEs:")
Expl_figure2_UserResponses

# run stats, helpfulness judgements:
item7t="Wilcox"
item7test=uk_wilcox.test(df_survey_Inconsist_responses$responseNo[df_survey_Inconsist_responses$group=="P"],df_survey_Inconsist_responses$responseNo[df_survey_Inconsist_responses$group=="C"],paired=FALSE,exact=FALSE)
item7effsize=item7test$z_val/(sqrt(nrow(df_survey_Inconsist_responses)))

matchingRes=paste(matchingRes,paste("\n","Expl_UserJudgementsInconsistencies",sep=""),"NA",item7t,item7test$p.value,item7effsize,sep = ",")

```

The analysis revealed:

* Is there a significant difference in terms of estimated usefulness for others between groups? We compared number of matches for users in plausible condition (M = `r df_survey_Inconsist_summary$mean[df_survey_Inconsist_summary$group=="P"]`, SD = `r df_survey_Inconsist_summary$sd[df_survey_Inconsist_summary$group=="P"]`) and users in the closest condition (M = `r df_survey_Inconsist_summary$mean[df_survey_Inconsist_summary$group=="C"]`, SD = `r df_survey_Inconsist_summary$sd[df_survey_Inconsist_summary$group=="C"]`) using a Wilcoxon–Mann–Whitney U test. This showed: U=`r item7test$statistic `, p=`r item7test$p.value `, r = `r item7effsize `
