...
# Where's the data
demo_source = list.files(path = "../BackEnd/userData", pattern="demographics.csv",full.names=TRUE)
perf_source = list.files(path = "../BackEnd/userData", pattern="performance.csv",full.names=TRUE)
rt_source = list.files(path = "../BackEnd/userData", pattern="reactionTime.csv",full.names=TRUE)
survey_source = list.files(path = "../BackEnd/userData", pattern="survey.csv",full.names=TRUE)
attention_source = list.files(path = "../BackEnd/userData", pattern="attentionCheck.csv",full.names=TRUE)
...
# identify users that answered with only 1 response type throughout all questions (very conservative)
...
H1.3_figure2_UserResponseMatches = ggplot(df_survey_relevantPlants_plot, aes(x=group, y=mean, fill=group)) + 
  ...
  theme_bw(base_size = 10)+
  scale_y_continuous(expand = c(0,0),limits = c(0,5),breaks=c(1:5))+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  theme(plot.title = element_text(hjust = 0.5))
...
p1 <- ggarrange(H1.1_p_ShubsPerTrial_summary_anno1,H1.2_p_RTPerTrial_summary_anno,ncol=1,nrow=2, common.legend = TRUE, legend = "bottom", labels = c("a", "b"), label.x=-0.001,label.y=1.04,font.label = list(size = 16),align = c("v"))
p2 <- ggarrange(H1.3_figure2_UserResponseMatches, 
                ncol = 1, nrow = 1,legend = "bottom", labels = c("c"),label.x=-0.001,label.y=1.02,font.label = list(size = 16))

H1_p_ShubsPerTrial_RTPerTrial = ggarrange(p1, "", p2, ncol = 3, nrow = 1, widths=c(75,5,35),common.legend = TRUE, legend = "bottom")

# save
ggsave("Figures/H1_p_ShubsPerTrial_RTPerTrial_PAZ_FINAL.pdf",width = 8,height = 4) #width = 7, height = 2,
```
