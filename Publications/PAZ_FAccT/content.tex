...
\citeauthor{wachter_counterfactual_2017} introduce a \gls{CFE} $\xcf\in\RN^\dimsym$ of a \gls{ML} model $\classifier:\RN^\dimsym\to\setY$ as an optimization problem~\citep{wachter_counterfactual_2017} :
...
The guiding question of the current work is whether \textit{computationally plausible \glspl{CFE}} have an advantage over \textit{closest \glspl{CFE}} in helping users to learn from a \gls{ML} model.
To assess this question, we rely on an interactive iterative learning task, where users repeatedly choose input values for a \gls{ML} model.
